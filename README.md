...
- **libgomp1** (Linux) — runtime Goose-бинаря (Rust + OpenMP). На Debian/Ubuntu:  
  `sudo apt-get install -y python3 python3-venv curl git tar bzip2 libgomp1`
- **~2 ГБ свободного места** на `.venv` + `.tools` + pip кеш

**Опционально (для Lean):**

- **Docker + Docker Compose v2** — только если нужна Lean 4 верификация
- **~16 ГБ RAM** — минимум для первой сборки Mathlib (1.5-2.5 часа)
- **~8 ГБ свободного места** на Docker volume с готовыми olean

### Поддерживаемые ОС

| ОС | Статус | Путь установки |
|---|---|---|
| **Linux** (Ubuntu 22+ / Debian 12+) | ✅ основная | `./setup.sh` |
| **macOS** 13+ (Intel/Apple Silicon) | ✅ | `./setup.sh` |
| **Windows 10/11 + WSL2** | ✅ через Linux-подсистему | `./setup.sh` внутри WSL |
| **Windows нативно** (PowerShell/cmd) | 🧪 beta | `setup.bat` + `python bin/ai4math.py` |

### Windows: рекомендуемый путь — WSL2

Если ты на Windows, **самый простой и надёжный вариант** — поставить WSL2 c Ubuntu и дальше работать как на Linux:

```powershell
# В PowerShell от администратора (однократно)
wsl --install -d Ubuntu
# Перезагрузиться, задать логин/пароль для WSL
```

Дальше **внутри WSL**:

```bash
sudo apt-get update && sudo apt-get install -y git curl python3 python3-venv bzip2 tar libgomp1
...
Docker Desktop for Windows с WSL2 backend тоже работает из-под Ubuntu — `./setup.sh --with-lean` поднимет `vendor/lean-checker` как обычно.

### Установка (Linux / macOS / WSL — 5 минут без Lean)
...
### Установка (Windows нативно, beta)
...
`setup.bat` делегирует к кросс-платформенному `setup.py` — проверяет Python 3.10+, создаёт venv, ставит зависимости, качает Goose Windows-бинарь (`goose.exe`), запускает wizard. Дальше:

```batch
python bin\ai4math.py                        :: интерактивная сессия
python bin\ai4math.py run "промпт"           :: одна задача
python bin\ai4math.py doctor                 :: проверка окружения
```

Нативный Windows путь помечен как beta — при проблемах используй WSL2 или [открой issue](https://github.com/andkhalov/AI4Math/issues). CI на `windows-latest` прогоняет setup + run task автоматически на каждый push.
