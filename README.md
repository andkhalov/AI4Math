...
`setup.bat` использует кросс-платформенный `setup.py` — проверяет Python 3.10+, создаёт venv, ставит зависимости, качает Goose Windows-бинарь (`goose.exe`), запускает wizard. Дальше:
...
⚠️ Нативный Windows путь на момент релиза протестирован меньше чем Linux. Если сталкиваешься с проблемами — или используй WSL2, или [открой issue](https://github.com/andkhalov/AI4Math/issues).
