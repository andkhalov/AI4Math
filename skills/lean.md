...
| `OFFLINE:` | Endpoint недоступен (сеть/TLS/таймаут). Не класс ошибки, а статус канала | См. раздел "OFFLINE handling" ниже |

## OFFLINE handling

Если `lean_check` или `lean_health` возвращают строку, начинающуюся с
`OFFLINE:` — верификационный endpoint временно недоступен (Tailscale
Funnel, сеть, cold start). Это **не** ошибка в твоём Lean-коде.

Правила:

1. **Не ретрай больше одного раза** внутри одной задачи — tool уже
   сделал внутренний retry с коротким таймаутом.
2. **Скажи пользователю одной фразой** что формальная проверка
   временно недоступна, и продолжи задачу без неё.
3. **Не блокируй выполнение задачи**, если она не чисто про Lean.
   Напиши код так, как ты считаешь правильным; отметь в ответе что
   код не прошёл формальную проверку из-за недоступности checker'а.
4. Если задача **полностью** про Lean-верификацию и без checker'а
   бессмысленна — предложи пользователю поднять локальный checker
   (`./setup.sh --with-lean-local`) и остановись.
5. На следующем витке диалога **всё равно пробуй** `lean_check`
   заново — endpoint может восстановиться.
