...
**Lean backend** по умолчанию — `https://scilibai.ru/grag` (SciLib-GRC21). Опциональный локальный fallback — `http://localhost:8888` (андkhalov/lean-checker, старая схема). MCP автоматически определяет нужную схему по URL.
...
- **SciLib-GRC21** (`https://scilibai.ru/grag`, [API docs](https://github.com/andkhalov/SciLib-GRC21/blob/main/docs/api.md)) — основной backend для Lean верификации (`/check`) и premise retrieval (`/search`). Lean 4.28-rc1 + Mathlib 4.26 + GraphDB + PostgreSQL + Qdrant.
